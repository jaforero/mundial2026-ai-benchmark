...
.h2h-row-compact{display:grid;grid-template-columns:80px 1fr 60px 60px 60px 60px 60px;gap:8px;padding:10px 12px;border-bottom:1px solid var(--border);align-items:center;font-size:12px;}
.h2h-row-compact:hover{background:var(--hover);cursor:pointer;}
.h2h-row-compact .date{color:var(--muted);font-weight:600;} .h2h-row-compact .teams{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.h2h-row-compact .score{font-weight:800;color:var(--deep-blue);text-align:center;} .h2h-row-compact .pred{font-size:11px;text-align:center;padding:2px 4px;border-radius:4px;}
...
let h2h_filters = {group: '', matchday: '', team: ''};
function h2hApplyFilters(){
  const gv = document.getElementById('h2h-group')?.value || '';
  const mv = document.getElementById('h2h-matchday')?.value || '';
  const tv = document.getElementById('h2h-team')?.value || '';
  h2h_filters = {group: gv, matchday: mv, team: tv.toLowerCase()};
  h2hRenderTable();
}
function h2hClearFilters(){ document.getElementById('h2h-group').value=''; document.getElementById('h2h-matchday').value=''; document.getElementById('h2h-team').value=''; h2h_filters={group:'',matchday:'',team:''}; h2hRenderTable(); }
function h2hRenderTable(){
  let rows = DATA.consensus.matches.filter(m=>{
    ...
    if(h2h_filters.matchday && (m.md||m.matchday)!==parseInt(h2h_filters.matchday)) return false;
  ...
  rows.forEach((m,i)=>{
    ...
    const sc = rz ? `${rz.ga}-${rz.gb}` : '—';
    html+=`<div class="h2h-row-compact" onclick="h2hShowDetail(${DATA.consensus.matches.indexOf(m)})">
      <div class="date">${m.date||'—'}</div>
      <div class="teams"><b>${tf(m.a)}</b> vs <b>${tf(m.b)}</b></div>
      <div class="score">${sc}</div>
      <div class="pred" style="background:${COL.Claude}20;color:${COL.Claude}">${c.score||'—'}</div>
      <div class="pred" style="background:${COL.ChatGPT}20;color:${COL.ChatGPT}">${ch.score||'—'}</div>
      <div class="pred" style="background:${COL.Gemini}20;color:${COL.Gemini}">${g.score||'—'}</div>
      <div class="pred" style="background:${COL.Consenso}20;color:${COL.Consenso}">${m.score||'—'}</div>
    </div>`;
  });
  html+=`</div><div style="margin-top:8px;font-size:11px;color:var(--muted);">${rows.length} de ${DATA.consensus.matches.length} partidos</div>`;
...
function renderConsenso(){
  ...
  <div class="card">
    ...
    <div class="h2h-filters">
      ...
      <div>
        <label>${tx('Jornada','Matchday')}</label>
        <select id="h2h-matchday" onchange="h2hApplyFilters()">
          <option value="">${tx('Todas','All')}</option>
          <option value="1">${tx('Jornada 1','Matchday 1')}</option>
          <option value="2">${tx('Jornada 2','Matchday 2')}</option>
          <option value="3">${tx('Jornada 3','Matchday 3')}</option>
        </select>
      </div>
      <div>
    ...
    <div style="font-size:11px;color:var(--muted);margin-top:12px;margin-bottom:8px;display:grid;grid-template-columns:80px 1fr 60px 60px 60px 60px 60px;gap:8px;padding:0 12px;font-weight:600;">
      <div>${tx('Fecha','Date')}</div><div>${tx('Partidos','Matches')}</div><div>${tx('Real','Result')}</div><div style="color:${COL.Claude}">Claude</div><div style="color:${COL.ChatGPT}">ChatGPT</div><div style="color:${COL.Gemini}">Gemini</div><div style="color:${COL.Consenso}">Consenso</div>
    </div>
    <div id="h2h-out"></div>
  </div>
