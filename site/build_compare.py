...
function renderConsenso(){
  ...
  <div class="card">
    ...
    <select class="h2h-sel" id="h2h-sel" onchange="h2hPick(this.value)">${h2hOptions()}</select>
...
function loadResults(){
  fetch('./results.json?ts='+Date.now())
    ...
    .then(j=>{ REAL_RESULTS=_normResults(j); renderAccuracy(); if(_h2hAuto&&document.getElementById('h2h-sel')) applyH2HDefault(); })
    .catch(()=>{ REAL_RESULTS=_normResults(RESULTS_FALLBACK); renderAccuracy(); if(_h2hAuto&&document.getElementById('h2h-sel')) applyH2HDefault(); });  // respaldo embebido
}
