...
# Hook de fuerza para sembrar el bracket (lo fija el motor v5: team -> Elo). Si es None, no siembra.
STRENGTH=None

def build_r32(winners,runners,thirds_by_group):
    """Bracket correcto del Mundial 2026: 32 clasificados -> 16 partidos, cada equipo UNA sola vez.
    Estructura: 8 ganadores vs 8 terceros, 4 ganadores vs 4 subcampeones, 8 subcampeones entre sí.
    Siembra por Elo (los ganadores más fuertes enfrentan a los terceros, el tier más débil) con
    salvaguarda de no cruzar equipos del mismo grupo. Garantiza que ninguna probabilidad de R32 > 100%."""
    S = STRENGTH or {}
    def s(t): return S.get(t, 1500.0)
    glist=list(GROUPS.keys())
    W=[winners[g] for g in glist]            # 12 ganadores
    R=[runners[g] for g in glist]            # 12 subcampeones
    T=list(thirds_by_group.values())         # 8 mejores terceros
    grp={}
    for g in glist: grp[winners[g]]=g; grp[runners[g]]=g
    for g,t in thirds_by_group.items(): grp[t]=g
    matches=[]
    # 8 ganadores más fuertes vs 8 terceros (más débil primero), evitando mismo grupo
    Ws=sorted(W,key=lambda t:-s(t)); w_thirds=Ws[:8]; w_runners=Ws[8:]
    Tpool=sorted(T,key=lambda t:s(t))
    for w in w_thirds:
        pick=next((t for t in Tpool if grp.get(t)!=grp.get(w)), Tpool[0])
        Tpool.remove(pick); matches.append((w,pick))
    # 4 ganadores restantes vs 4 subcampeones (más débiles), evitando mismo grupo
    Rpool=sorted(R,key=lambda t:s(t))
    for w in w_runners:
        pick=next((t for t in Rpool if grp.get(t)!=grp.get(w)), Rpool[0])
        Rpool.remove(pick); matches.append((w,pick))
    # 8 subcampeones restantes entre sí (fuerte vs débil)
    rem=sorted(Rpool,key=lambda t:-s(t)); i,j=0,len(rem)-1
    while i<j:
        matches.append((rem[i],rem[j])); i+=1; j-=1
    return matches
