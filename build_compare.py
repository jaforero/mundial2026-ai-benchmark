...
function renderAccuracy(){
  ...
  if(nClosed>0){
    ...
    const cBoard=ACC_MODELS.map(m=>({...m,kpts:(koAgg[m.key]||{pts:0}).pts,mpts:agg[m.key].pts,spts:sAgg[m.key].pts}))
      ...
      .sort((a,b)=>b.tot-a.tot||b.kpts-a.kpts||b.mpts-a.mpts);   // desempate: 1º eliminatorias, 2º partidos de grupos
    const tieTop=cBoard.length>1&&cBoard[0].tot===cBoard[1].tot;
    const tieTxt=tieTop?tx(' <b>Empate en la cima ('+cBoard[0].tot+' pts)</b>, resuelto por puntos de eliminatorias.',' <b>Tied at the top ('+cBoard[0].tot+' pts)</b>, broken by knockout points.'):'';
    const cCards=cBoard.map((x,i)=>`<div class="lb-card ${i===0?'lead':''}"><div class="rank">${medals[i]||''}</div><div class="who"><span class="dotc" style="background:${x.color}"></span>${x.name}</div><div class="pts">${x.tot}<small> ${tx('pts','pts')}</small></div><div class="mini"><span><b>${x.kpts}</b>${tx('eliminatorias','knockouts')}</span><span><b>${x.mpts}</b>${tx('partidos grupos','group matches')}</span><span><b>${x.spts}</b>${tx('posiciones','standings')}</span></div></div>`).join('');
