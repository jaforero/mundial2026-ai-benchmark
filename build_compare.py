...
.lb-card.refcard{border-style:dashed;border-color:var(--muted);opacity:.96;background:linear-gradient(165deg,rgba(120,120,140,.05),transparent);}
.ref-pill{font-size:8px;font-weight:800;letter-spacing:.06em;color:var(--muted);border:1px dashed currentColor;border-radius:8px;padding:2px 6px;white-space:nowrap;}
...
.hl-badge{margin:0 0 22px;padding:0;border-radius:18px;overflow:hidden;border:1px solid rgba(78,0,255,.25);box-shadow:0 8px 30px rgba(78,0,255,.13);background:var(--card);}
.hl-head{background:linear-gradient(120deg,#4e00ff,#0048ff 55%,#041c59);padding:16px 20px;color:#fff;position:relative;overflow:hidden;}
.hl-head::after{content:'';position:absolute;top:-40%;right:-6%;width:180px;height:180px;background:radial-gradient(circle,rgba(255,255,255,.16),transparent 70%);}
.hl-tag{display:inline-block;font-size:10.5px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;background:rgba(255,255,255,.18);padding:3px 11px;border-radius:20px;margin-bottom:9px;}
.hl-title{font-size:19px;font-weight:900;line-height:1.25;margin:0;letter-spacing:-.2px;}
.hl-sub{font-size:12.5px;opacity:.92;margin-top:5px;line-height:1.5;font-weight:500;max-width:640px;}
.hl-body{padding:16px 20px 18px;}
.hl-flags{display:flex;flex-wrap:wrap;justify-content:center;gap:10px;margin-bottom:16px;}
.hl-flag{display:flex;flex-direction:column;align-items:center;gap:4px;min-width:82px;padding:11px 8px 9px;background:linear-gradient(160deg,rgba(78,0,255,.07),rgba(0,72,255,.04));border:1px solid var(--border);border-radius:13px;transition:transform .15s;}
.hl-flag:hover{transform:translateY(-3px);}
.hl-flag-e{font-size:40px;line-height:1;filter:drop-shadow(0 3px 7px rgba(4,28,89,.32));}
.hl-flag-n{font-size:12px;font-weight:800;color:var(--deep-blue);}
.hl-flag-r{font-size:9.5px;font-weight:700;color:var(--purple);letter-spacing:.3px;}
.hl-tbl{width:100%;border-collapse:collapse;font-size:11.5px;margin-top:4px;}
.hl-tbl th{text-align:center;font-size:9.5px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);padding:6px 8px;border-bottom:1.5px solid var(--border);}
.hl-tbl th:first-child{text-align:left;}
.hl-tbl td{padding:7px 8px;text-align:center;border-bottom:1px solid rgba(0,0,0,.05);}
.hl-tbl td:first-child{text-align:left;font-weight:800;color:var(--deep-blue);}
...
function koPredHTML(field,color){
  ...
  const rows=KR.map(s=>{const p=s[field]; if(!p)return '';
    ...
    if(ph!==_kph){_kph=ph;const _PN={'R16':['Octavos','Round of 16'],'R32':['Dieciseisavos','Round of 32'],'QF':['Cuartos','Quarterfinals'],'SF':['Semifinales','Semifinals'],'3P':['Tercer puesto','Third place'],'F':['Final','Final']};const nm=_PN[ph]?tx(_PN[ph][0],_PN[ph][1]):ph;div=`<tr><td colspan="5" style="padding:9px 8px 3px;font-size:10.5px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--purple)">${nm}</td></tr>`;}
...
function koScoreHTML(){
  ...
  const medals=['🥇','🥈','🥉',''];
  const board=ACC_MODELS.map(m=>({...m,...agg[m.key]})).filter(x=>x.n>0).sort((a,b)=>b.pts-a.pts||b.exact-a.exact||b.qual-a.qual);
  const cards=refSplit(board).map((x,i)=>lbCard(x,i,x.key==='consenso',`<div class="who"><span class="dotc" style="background:${x.color}"></span>${x.name}</div><div class="pts">${x.pts}<small> ${tx('pts','pts')}</small></div><div class="mini"><span><b>${x.qual}/${x.n}</b>${tx('clasificados','qualifiers')}</span><span><b>${x.exact}</b>${tx('exactos','exact')}</span></div>`)).join('');
  ...
  const PHN={'R32':[ 'Dieciseisavos','Round of 32'],'R16':['Octavos','Round of 16'],'QF':['Cuartos','Quarterfinals'],'SF':['Semifinales','Semifinals'],'3P':['Tercer puesto','Third place'],'F':['Final','Final']};
  const RANK={'F':5,'3P':4.5,'SF':4,'QF':3,'R16':2,'R32':1};
  ...
  // Fases en juego (final y 3er puesto) expandidas arriba; todas las fases cerradas plegadas en <details>,
  // cada una con el resumen de puntos de su ronda — mismo patrón que ya usaban los dieciseisavos.
  const OPEN=['F','3P'];
  const hi=rows.filter(r=>OPEN.includes(r.ph)).sort((a,b)=>(RANK[b.ph]||0)-(RANK[a.ph]||0)||(+a.code.slice(1))-(+b.code.slice(1)));
  let _ph=null, hiRows='';
  hi.forEach(r=>{if(r.ph!==_ph){_ph=r.ph;hiRows+=sepOf(r.ph);}hiRows+=mkRow(r);});
  const foldPhase=(ph,tot)=>{
    const fr=rows.filter(r=>r.ph===ph).sort((a,b)=>(+a.code.slice(1))-(+b.code.slice(1)));
    if(!fr.length) return '';
    const pts=ACC_MODELS.map(m=>{let p=0;fr.forEach(r=>{const c=r[m.key];if(c)p+=c.tot;});return `${m.name} ${p}`;}).join(' · ');
    const nm=PHN[ph]?PHN[ph][LANG==='en'?1:0]:ph;
    return `<details class="phase-closed-d" style="margin-top:12px"><summary style="cursor:pointer;font-weight:800;font-size:12.5px">📁 ${nm} · ${tx('fase cerrada','closed phase')} (${fr.length}/${tot}) — ${tx('puntos de la ronda','round points')}: ${pts}</summary><div class="h2h-table" style="margin-top:8px">${fr.map(mkRow).join('')}</div></details>`;
  };
  const folded=[['SF',2],['QF',4],['R16',8],['R32',16]].map(x=>foldPhase(x[0],x[1])).join('');
  const hiBlock=hiRows?`${koHead}<div class="h2h-table">${hiRows}</div>`:'';
...
function renderAccuracy(){
  ...
  const board=ACC_MODELS.map(m=>{const A=agg[m.key];return{...m,...A,
    ...
    .filter(x=>x.n>0).sort((a,b)=> b.pts-a.pts || b.exact-a.exact || a.avgRps-b.avgRps);
  const medals=['🥇','🥈','🥉',''];
  const cards=refSplit(board).map((x,i)=>lbCard(x,i,x.key==='consenso',`
...
function bkHighlightHTML(){
  // Hallazgo: los 4 semifinalistas reales estaban en el top de las proyecciones pre-Mundial de las 3 IAs.
  // Datos EXACTOS verificados (prob. de campeón pre-Mundial + posición en el ranking de cada IA).
  const sfR=(DATA.ko_real||[]).filter(s=>s.phase==='SF');
  if(sfR.length<2) return '';
  const teams=['España','Francia','Argentina','Inglaterra'];
  const FL={'España':'🇪🇸','Francia':'🇫🇷','Argentina':'🇦🇷','Inglaterra':'🏴󠁧󠁢󠁥󠁮󠁧󠁿'};
  const champ={}; // ia -> ranking array
  ['claude','chatgpt','gemini'].forEach(ia=>{
    const c=((DATA[ia]||{}).reach||{}).CAMPEON||{};
    champ[ia]=Object.entries(c).sort((a,b)=>b[1]-a[1]).map(x=>x[0]);
  });
  const pos=(ia,t)=>{const i=(champ[ia]||[]).indexOf(t);return i<0?'—':i+1;};
  const flagCards=teams.map(t=>`<div class="hl-flag" title="${tn(t)}"><span class="hl-flag-e">${FL[t]}</span><span class="hl-flag-n">${tn(t)}</span><span class="hl-flag-r">${tx('semifinalista','semifinalist')}</span></div>`).join('');
  const rows=teams.map(t=>{
    const cell=ia=>{const p=pos(ia,t);const cls=(typeof p==='number'&&p<=4)?'top4':'out';return `<td><span class="hl-pos ${cls}">${p}º</span></td>`;};
    return `<tr><td>${FL[t]} ${tn(t)}</td>${cell('claude')}${cell('chatgpt')}${cell('gemini')}</tr>`;
  }).join('');
  return `<div class="hl-badge">
    <div class="hl-head">
      <span class="hl-tag">🎯 ${tx('Hallazgo clave','Key finding')}</span>
      <h3 class="hl-title">${tx('Las tres IAs anticiparon a los 4 semifinalistas','All three AIs foresaw the 4 semifinalists')}</h3>
      <p class="hl-sub">${tx('Antes de que rodara el balón, España, Francia, Argentina e Inglaterra ya figuraban en lo más alto de las proyecciones de título de las tres IAs. La realidad del torneo confirmó ese cuarteto en semifinales.','Before a ball was kicked, Spain, France, Argentina and England already sat at the top of all three AIs\\u2019 title projections. The tournament confirmed that quartet in the semifinals.')}</p>
    </div>
    <div class="hl-body">
      <div class="hl-flags">${flagCards}</div>
      <table class="hl-tbl">
        <thead><tr><th>${tx('Selección','Team')}</th><th>Claude</th><th>ChatGPT</th><th>Gemini</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
